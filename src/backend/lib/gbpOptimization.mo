...
module {
  public type GbpOptimization = Types.GbpOptimization;
  ...
  public func save(state : State, item : GbpOptimization) : () {
    state.items.add(item.id, item);
  };

  public func get(state : State, id : Text) : ?GbpOptimization {
    state.items.get(id);
  };

  public func update(state : State, id : Text, item : GbpOptimization) : Bool {
  ...
  public func listByClient(state : State, clientBusinessId : Text) : [GbpOptimization] {
    let results = Map.empty<Text, GbpOptimization>();
