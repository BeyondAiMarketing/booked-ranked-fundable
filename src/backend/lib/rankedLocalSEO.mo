...
module {
  public type RankedLocalSEO = Types.RankedLocalSEO;
  ...
  public func save(state : State, item : RankedLocalSEO) : () {
    state.items.add(item.id, item);
  };

  public func get(state : State, id : Text) : ?RankedLocalSEO {
    state.items.get(id);
  };

  public func update(state : State, id : Text, item : RankedLocalSEO) : Bool {
  ...
  public func listByClient(state : State, clientBusinessId : Text) : [RankedLocalSEO] {
    let results = Map.empty<Text, RankedLocalSEO>();
