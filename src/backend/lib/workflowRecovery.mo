...
module {
  public type WorkflowRecovery = Types.WorkflowRecovery;
  ...
  public func save(state : State, item : WorkflowRecovery) : () {
    state.items.add(item.id, item);
  };

  public func get(state : State, id : Text) : ?WorkflowRecovery {
    state.items.get(id);
  };

  public func update(state : State, id : Text, item : WorkflowRecovery) : Bool {
  ...
  public func listByClient(state : State, clientBusinessId : Text) : [WorkflowRecovery] {
    let results = Map.empty<Text, WorkflowRecovery>();
