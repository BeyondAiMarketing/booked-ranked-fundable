import RagBrainLib "../libraries/ragBrain";
...
mixin (
  ...
  public func queryRAG(
    ...
    let mappedChunks = Array.map<RagBrainTypes.DocumentChunk, { id : Text; content : Text; chunkIndex : Nat }>(
      result.chunks,
  ...
  public query func getKnowledgeDocuments(
    ...
    let allDocs = Map.toArray(ragBrainState.documents);
    let filtered = Array.filter<(Text, RagBrainTypes.KnowledgeDocument)>(
      allDocs,
      func(pair) {
        let d = pair.1;
    ...
    Array.map<(Text, RagBrainTypes.KnowledgeDocument), { id : Text; title : Text; chunkCount : Nat; uploadedAt : Int }>(
      filtered,
  ...
  public query func getVectorIndexStatus() : async { totalChunks : Nat; totalDocuments : Nat; collectionsCount : Nat } {
    let chunks = List.toArray(ragBrainState.chunks);
    let docs = Map.toArray(ragBrainState.documents);
  ...
  public query func getAIUsageLogs(
    ...
    Array.map<RagBrainTypes.AIUsageLog, { id : Text; provider : Text; inputTokens : Nat; success : Bool; loggedAt : Int }>(
      logs,
  ...
  public query func getConversationHistory(
    ...
    Array.map<RagBrainTypes.ConversationMessage, { id : Text; role : Text; content : Text; citations : [Text]; timestamp : Int }>(
      msgs,
