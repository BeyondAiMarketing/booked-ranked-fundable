import { ad as useActor, r as reactExports, j as jsxRuntimeExports } from "./index-Dwzp0QDY.js";
import { u as useBusinessBrief } from "./useBusinessBrief-DB4dL_cV.js";
import { u as useVerticalProfile } from "./useVerticalProfile-NJ6iI9sA.js";
function useWorkflowLog() {
  ...
  reactExports.useEffect(() => {
    if (!actor) return;
    setLoading(true);
    actor.getWorkflowLogsByWorkflow("all").then((result) => {
      if ("ok" in result) {
        setLogs(result.ok);
      } else {
        setError(result.err);
